    ...
    if(fromModel)
    {
        if(toModel)
        {
            ...
            for (unsigned int i=0; i<fromModel->getNbHexahedra(); ++i)
            {
                ...
                std::pair<std::map<fixed_array<index_type,2>, int>::iterator, bool> insert_result;
                ...
                std::pair<std::map<fixed_array<index_type,4>, int>::iterator, bool> insert_facets_result;
