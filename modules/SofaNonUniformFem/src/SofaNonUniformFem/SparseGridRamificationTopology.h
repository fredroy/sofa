...
// 				virtual int findNearestCube(const Vector3& pos);
    ...
    struct Connexion
    {
        ...
        std::array< std::set<Connexion*>,NUM_CONNECTED_NODES >	_neighbors;	// the connexion graph at a given level (it can have several neighbors in each direction)
...
public :
    ...
    typedef std::vector<std::array<helper::vector<int>,8> > HierarchicalCubeMapRamification; ///< a cube indice -> corresponding child indices (possible more than 8 for Ramification)
