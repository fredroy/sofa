...
public:
    ...
    typedef std::array< InCoord, 8 > Nodes;
...
// 	  helper::vector< helper::vector<unsigned > > _pointsCorrespondingToElem; // in which element is the interpolated vertex?
    ...
    // for method 2
    helper::vector< std::pair< int, std::array<Real,8> > > _finestBarycentricCoord; // barycentric coordinates for each mapped points into the finest elements (fine element idx + weights)
