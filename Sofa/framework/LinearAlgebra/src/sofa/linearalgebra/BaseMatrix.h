...
public:
    ...
    void add(Index row, Index col, const Eigen::MatrixBase<Derived>& M)
    {
        for (unsigned int r = 0; r < Derived::RowsAtCompileTime; ++r)
        {
            for (unsigned int c = 0; c < Derived::ColsAtCompileTime; ++c)
            {
                this->add(row + r, col + c, M(r,c));
            }
        }
    }
