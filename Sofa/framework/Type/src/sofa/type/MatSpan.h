...
public:
    ...
    using VecSpan = std::span<real, C>;
    
    std::array<real, L * C> elems;
    std::span<real, L * C> elemsSpan = std::span(elems);
    ...
    typedef typename VecSpan::iterator        iterator;
    typedef typename VecSpan::iterator        const_iterator; // const_iterator in c++23
    typedef typename VecSpan::reference       reference;
    typedef typename VecSpan::const_reference const_reference;
    ...
    constexpr MatSpan() noexcept = default;
    ...
    constexpr MatSpan(std::initializer_list<std::initializer_list<Real>>&& rows) noexcept
    {
        assert(rows.size() == L);
        ...
        for (const auto& row : rows)
        {
            ...
            for (auto scalar : row)
            {
                this->elems[rowId * C + colId++] = scalar;
            }
            ++rowId;
        }
    }
    ...
    >
    constexpr MatSpan(ArgsT&&... r) noexcept
        : elems{ std::forward<ArgsT>(r)... }
    {}

    /// Constructor from an element
    explicit constexpr MatSpan(const real& v) noexcept
    {
        for( Size i=0; i<L; i++ )
            for( Size j=0; j<C; j++ )
                this->elems[i*C+j] = v;
    ...
    constexpr MatSpan(const MatSpan<L,C,real2>& m) noexcept
    {
    ...
    explicit constexpr MatSpan(const MatSpan<L2,C2,real2>& m) noexcept
    {
        ...
        for( Size l=0 ; l<minL ; ++l )
        {
            for( Size c=0 ; c< minC; ++c )
                this->elems[l * C + c] = static_cast<real>(m[l * minL + c]);
    ...
    template<typename real2>
    explicit constexpr MatSpan(const real2* p) noexcept
    {
        ...
        else
        {
            for (Size l = 0; l < L; ++l)
                for (Size c = 0; c < C; ++c)
                    this->elems[l*C + c] = static_cast<real>(p[l*C + c]);
        }
    }
    ...
    template<typename real2>
    constexpr void operator=(const MatSpan<L,C,real2>& m) noexcept
    {
        std::copy(m.begin(), m.begin()+L, this->begin());
    }
    ...
    constexpr void operator=(const MatSpan<L2,C2,real>& m) noexcept
    {
        std::copy(m.begin(), m.begin()+(L>L2?L2:L), this->begin());
    }

    template<Size L2, Size C2>
    constexpr void getsub(Size L0, Size C0, MatSpan<L2,C2,real>& m) const noexcept
    {
        for (Size i=0; i<L2; i++)
            for (Size j=0; j<C2; j++)
                m[i*C2 + j] = this->elems[ (i+L0) * C + (j+C0)];
    }
//
//    template <Size C2>
...
//
    ...
    {
        std::fill_n(this->elems.begin(), this->elems.size(), 0);
    }
...
//    }
    ...
    constexpr MatSpan<L,C,real> operator+(const MatSpan<L,C,real>& m) const noexcept
    {
        MatSpan<L,C,real> r;
        for(Size i = 0; i < L; i++)
            for(Size j = 0; i < C; i++)
                r[i*C +j] = (*this)[i*C +j] + m[i*C +j];
        return r;
    }
//
...
//    constexpr MatSpan<L,C,real> operator-() const noexcept
//    {
//        Mat<L,C,real> r(NOINIT);
//        for(Size i = 0; i < L; i++)
//            r[i] = -(*this)[i];
...
//
    /// invert this
    template<sofa::Size NbLine = L, sofa::Size NbColumn = C, typename = std::enable_if_t<NbLine == NbColumn> >
    [[nodiscard]] constexpr MatSpan<L,C,real> inverted() const
    {
        static_assert(L == C, "Cannot invert a non-square matrix");
        MatSpan<L,C,real> m = *this;
...
[[nodiscard]] bool invertMatrix(MatSpan<S,S,real>& dest, const MatSpan<S,S,real>& from)
{
    sofa::Size i{0}, j{0}, k{0};
    std::array<sofa::Size, S> r, c, row, col;

    MatSpan<S,S,real> m1 = from;
    MatSpan<S,S,real> m2;
    m2.identity();
...
constexpr MatSpan<L,P,real> operator*(const MatSpan<L,C,real>& m1, const MatSpan<C,P,real>& m2) noexcept
{
    MatSpan<L,P,real> r;
    for (Size i = 0; i<L; i++)
    {
        for (Size j = 0; j<P; j++)
        {
            r[i * C + j] = m1[i * C + 0] * m2[0 * C +j];
            for (Size k = 1; k<C; k++)
            {
                r[i * C + j] += m1[i * C + k] * m2[k * C + j];
            }
        }
    }
    return r;
