...
void OglViewport::draw(const core::visual::VisualParams* vparams)
{
    ...
    const auto vecsize = Vec3f(0.1f,0.1f,0.1f);
	if (!p_cameraRigid.isDisplayed())
        vparams->drawTool()->drawFrame(p_cameraPosition.getValue(), p_cameraOrientation.getValue(), vecsize);
	else
	{
		RigidCoord rcam = p_cameraRigid.getValue();
        vparams->drawTool()->drawFrame(rcam.getCenter(), rcam.getOrientation(), vecsize);
	}
}
