    ...
    if (mesh->getNbTetrahedra()+mesh->getNbHexahedra() > 0)
    {
        if (mesh->getNbTetrahedra()>0)
        {
            ...
            for (sofa::Index i=0; i < elems.size(); ++i)
            {
                ...
                Coord n;
                n = type::cross(p3-p1,p2-p1); n.normalize();
                n1 += n;
                n2 += n;
                n3 += n;
                n = type::cross(p4-p1,p3-p1); n.normalize();
                n1 += n;
                n3 += n;
                n4 += n;
                n = type::cross(p2-p1,p4-p1); n.normalize();
                n1 += n;
                n4 += n;
                n2 += n;
                n = type::cross(p3-p2,p4-p2); n.normalize();
                n2 += n;
                n4 += n;
                n3 += n;
            }
    ...
    else if (mesh->getNbTriangles()+mesh->getNbQuads() > 0)
    {
        if (mesh->getNbTriangles()>0)
        {
            ...
            for (sofa::Index i=0; i < elems.size(); ++i)
            {
                ...
                Coord& n3 = normals[e[2]];
                Coord n;
                n = type::cross(p2-p1,p3-p1); n.normalize();
                n1 += n;
                n2 += n;
                n3 += n;
            }
        ...
        if (mesh->getNbQuads()>0)
        {
            ...
            for (sofa::Index i=0; i < elems.size(); ++i)
            {
                ...
                Coord& n4 = normals[e[3]];
                Coord n;
                n = type::cross(p3-p1,p4-p2); n.normalize();
                n1 += n;
                n2 += n;
                n3 += n;
                n4 += n;
