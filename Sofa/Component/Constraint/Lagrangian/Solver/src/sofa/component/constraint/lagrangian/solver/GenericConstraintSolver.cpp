...
GenericConstraintSolver::GenericConstraintSolver()
    ...
    , tolerance( initData(&tolerance, static_cast<SReal>(0.001), "tolerance", "residual error threshold for termination of the Gauss-Seidel algorithm"))
    , sor( initData(&sor, static_cast<SReal>(1.0), "sor", "Successive Over Relaxation parameter (0-2)"))
    ...
    , currentError(initData(&currentError, static_cast<SReal>(0.0), "currentError", "OUTPUT: current error"))
