...
LCPConstraintSolver::LCPConstraintSolver()
    ...
    , tol( initData(&tol, static_cast<SReal>(0.001), "tolerance", "residual error threshold for termination of the Gauss-Seidel algorithm"))
    , maxIt( initData(&maxIt, 1000, "maxIt", "maximal number of iterations of the Gauss-Seidel algorithm"))
    , mu( initData(&mu, static_cast<SReal>(0.6), "mu", "Friction coefficient"))
    , minW( initData(&minW, static_cast<SReal>(0.0), "minW", "If not zero, constraints whose self-compliance (i.e. the corresponding value on the diagonal of W) is smaller than this threshold will be ignored"))
    , maxF( initData(&maxF, static_cast<SReal>(0.0), "maxF", "If not zero, constraints whose response force becomes larger than this threshold will be ignored"))
