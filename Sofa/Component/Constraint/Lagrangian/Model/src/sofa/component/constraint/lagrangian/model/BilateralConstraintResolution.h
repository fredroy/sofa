...
public:
    ...
    , wBlock(Eigen::MatrixX<SReal>(blockSize, blockSize))
    {
    }
    void init(int line, SReal** w, SReal * /*force*/) override
    {
        ...
        {
            wBlock.row(i) = Eigen::VectorX<SReal>::Map(&w[line + i][line], wBlock.cols());
        }
    ...
    {
        Eigen::Map< Eigen::VectorX<SReal> > f(&force[line], wBlock.cols());
        Eigen::Map< Eigen::VectorX<SReal> > d(&displacement[line], wBlock.cols());
        Eigen::VectorX<SReal> f_local = wBlockInv.solve(d);
        f -= f_local;
    }

protected:
    Eigen::MatrixX<SReal>  wBlock;
    Eigen::LDLT< Eigen::MatrixX<SReal> > wBlockInv;
};
