...
void ConstraintProblem::gaussSeidelConstraintTimed(SReal &timeout, int numItMax)
{
    ...
    SReal t0 = (SReal)_timer->getTime() ;
    SReal timeScale = 1.0 / (SReal)CTime::getTicksPerSec();
...
ConstraintAnimationLoop::ConstraintAnimationLoop(simulation::Node* gnode)
    ...
    , d_tol( initData(&d_tol, static_cast<SReal>(0.00001), "tolerance", "Tolerance of the Gauss-Seidel"))
    ...
    , d_sor( initData(&d_sor, static_cast < SReal>(1.0), "sor","Successive Over Relaxation parameter (0-2)"))
