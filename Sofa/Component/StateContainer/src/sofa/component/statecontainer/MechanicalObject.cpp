...
void MechanicalObject<defaulttype::Rigid3Types>::draw(const core::visual::VisualParams* vparams)
{
    ...
    if (showObject.getValue())
    {
        ...
        for (size_t i = 0; i < vsize; ++i)
        {
            ...
            const auto frameSize = type::Vec3f ( 1.f,1.f,1.f );

			if (getContext()->isSleeping())
			{
                vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::gray());
			}
			else switch( drawMode.getValue() )
            {
                case 1:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::green());
                    break;
                case 2:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::red());
                    break;
                case 3:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::blue());
                    break;
                case 4:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::yellow());
                    break;
                case 5:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::magenta());
                    break;
                case 6:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize, sofa::type::RGBAColor::cyan());
                    break;
                default:
                    vparams->drawTool()->drawFrame ( type::Vec3(), type::Quat<SReal>(), frameSize );
            }
