...
void TriangularFEMForceFieldOptim<DataTypes>::initTriangleInfo(Index i, TriangleInfo& ti, const Triangle t, const VecCoord& x0)
{
    ...
    ti.bx = type::dot(ti.init_frame.row(0) , ab);
    ti.cx = type::dot(ti.init_frame.row(0) , ac);
    ti.cy = type::dot(ti.init_frame.row(1) , ac);
...
void TriangularFEMForceFieldOptim<DataTypes>::addForce(const core::MechanicalParams* /* mparams */, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    ...
    for ( Index i=0; i<nbTriangles; i+=1)
    {
        ...
        Real dbx = ti.bx - type::dot(ts.frame.row(0) , ab);
        Real dcx = ti.cx - type::dot(ts.frame.row(0) , ac);
        Real dcy = ti.cy - type::dot(ts.frame.row(1) , ac);
        ...
        Deriv fb = ts.frame.row(0) * (ti.cy * stress[0] - ti.cx * stress[2])  // (cy,   0, -cx) * stress
                + ts.frame.row(1) * (ti.cy * stress[2] - ti.cx * stress[1]);  // ( 0, -cx,  cy) * stress
        Deriv fc = ts.frame.row(0) * (ti.bx * stress[2])                      // ( 0,   0,  bx) * stress
                + ts.frame.row(1) * (ti.bx * stress[1]);                      // ( 0,  bx,   0) * stress
        Deriv fa = -fb-fc;
...
void TriangularFEMForceFieldOptim<DataTypes>::addDForce(const core::MechanicalParams* mparams, DataVecDeriv& d_df, const DataVecDeriv& d_dx)
{
    ...
    for ( Index i=0; i<nbTriangles; i+=1)
    {
        ...
        Real dbx = type::dot(ts.frame.row(0),dab);
        Real dby = type::dot(ts.frame.row(1),dab);
        Real dcx = type::dot(ts.frame.row(0),dac);
        Real dcy = type::dot(ts.frame.row(1),dac);
        ...
        Deriv dfb = ts.frame.row(0) * (ti.cy * dstress[0] - ti.cx * dstress[2])  // (cy,   0, -cx) * stress
            + ts.frame.row(1) * (ti.cy * dstress[2] - ti.cx * dstress[1]);       // ( 0, -cx,  cy) * stress
        Deriv dfc = ts.frame.row(0) * (ti.bx * dstress[2])                       // ( 0,   0,  bx) * stress
            + ts.frame.row(1) * (ti.bx * dstress[1]);                            // ( 0,  bx,   0) * stress
        Deriv dfa = -dfb - dfc;
...
void TriangularFEMForceFieldOptim<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
    ...
    else
    {
        ...
        for (unsigned int i=0; i<nbTriangles; ++i)
        {
            ...
            const TriangleState& ts = triState[i];
            const auto frame = ts.frame.transpose(); // check if frame should be Mat<3,2> instead
            Coord a = x[t[0]];
            Coord b = x[t[1]];
            Coord c = x[t[2]];
            Coord fx = ts.frame.row(0);
            Coord fy = ts.frame.row(1);
            Vec3 center = type::toVec3((a+b+c)*(1.0_sreal/3.0_sreal));
            Real scale = (Real)(sqrt((b-a).cross(c-a).norm()*0.25f));
            points[0].push_back(center);
            points[0].push_back(center + frame.col(0) * scale);
            points[1].push_back(center);
            points[1].push_back(center + frame.col(1) * scale);
