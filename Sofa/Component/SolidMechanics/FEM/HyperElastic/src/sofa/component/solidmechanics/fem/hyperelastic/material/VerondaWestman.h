...
{
public:
    static constexpr std::string_view Name = "VerondaWestman";
                         ...
                         const MaterialParameters<DataTypes>& param) override
    {
        ...
        return c1*(exp(c2*(pow(sinfo->J, (Real)(-2.0/3.0))*I1-3))-1)-c1*c2*(
                   pow(sinfo->J, (Real)(-4.0/3.0))*I2-3)/2+k0*log(sinfo->J)*log(sinfo->J)/2;
    }
                         ...
                         MatrixSym& SPKTensorGeneral) override
    {
        ...
        MatrixSym ID;
        ID.identity();
        SPKTensorGeneral = (ID-inversematrix*I1/3)*2*c1*c2*pow(sinfo->J, (Real)(-2.0/3.0))*exp(
                               c2*(pow(sinfo->J, (Real)(-2.0/3.0))*I1-3))
                           +(ID*I1-C-inversematrix*I2*2/3)*(-c1*c2*pow(sinfo->J, (Real)(-4.0/3.0)))+
                               ...
                               const MatrixSym& inputTensor, MatrixSym& outputTensor) override
    {
        ...
        outputTensor = (inversematrix*I1*_trHC/(Real)9.0-inversematrix*trH/(Real)3.0+Firstmatrix/(
                            Real)3.0*I1-ID*_trHC/(Real)3.0)*(Real)2.0*c1*c2*pow(
                           sinfo->J, (Real)(-2.0/3.0))*exp(
                           c2*(I1*pow(sinfo->J, (Real)(-2.0/3.0))-(Real)3.0))
                       +(ID-inversematrix*I1/(Real)3.0)*exp(
                           c2*(I1*pow(sinfo->J, (Real)(-2.0/3.0))-(Real)3))*(Real)2*c1*c2*c2*pow(
                           sinfo->J, (Real)(-4.0/3.0))*(-I1*_trHC/(Real)3.0+trH)
                       +(inversematrix*(Real)(-4.0)*_trHC*I2/9.0+inversematrix*(Real)2.0*I1*trH/3.0-
                         inversematrix*(Real)2.0*trHC/3.0-Firstmatrix*(Real)2.0*I2/3.0+ID*_trHC*I1*(
                             Real)(2.0/3.0)-ID*trH-C*_trHC*(Real)(2.0/3.0)+inputTensor)*c1*c2*pow(
                          ...
                          const MaterialParameters<DataTypes>& param, Matrix6& outputTensor) override
    {
        ...
        C[1] += sinfo->deformationTensor[1];
        C[3] += sinfo->deformationTensor[3];
        C[4] += sinfo->deformationTensor[4];
        Matrix6 C_H_C;
        C_H_C(0,0) = _C[0]*_C[0];
        ...
        Real I1square = (Real)(sinfo->deformationTensor[0]*sinfo->deformationTensor[0]+sinfo->
                               ...
                               deformationTensor[5]*sinfo->deformationTensor[5]+
                               2*(sinfo->deformationTensor[1]*sinfo->deformationTensor[1]+sinfo->
                                  deformationTensor[3]*sinfo->deformationTensor[3]+sinfo->
                                  deformationTensor[4]*sinfo->deformationTensor[4]));
        Real I2 = (Real)((pow(I1, (Real)2)-I1square)/2);
        Real c1 = param.parameterArray[0];
        Real c2 = param.parameterArray[1];
        Real k0 = param.parameterArray[2];
        ...
        outputTensor = ((trC_HC_*I1/(Real)9.0-trC_HID/(Real)3.0+C_H_C/(Real)3.0*I1-trID_HC_/(Real)
                         3.0)*(Real)2.0*c1*c2*pow(sinfo->J, (Real)(-2.0/3.0))*exp(
                            c2*(I1*pow(sinfo->J, (Real)(-2.0/3.0))-(Real)3.0))
                        +(trC_HC_*I1*I1/(Real)9.0-trC_HID*I1/(Real)3.0-trID_HC_*I1/(Real)3.0+
                          trIDHID)*(Real)2*c1*c2*c2*pow(sinfo->J, (Real)(-4.0/3.0))*exp(
                            c2*(I1*pow(sinfo->J, (Real)(-2.0/3.0))-(Real)3))
                        +(trC_HC_*(-1.0)*(Real)4.0*I2/(Real)9.0+trC_HID*(Real)2.0*I1/(Real)3.0-
                          trC_HC*(Real)2.0/(Real)3.0-C_H_C*(Real)2.0*I2/(Real)3.0+trID_HC_*I1*(Real)
                          (2.0/3.0)-trIDHID-trCH_C*(Real)(2.0/3.0)+IDHID)*c1*c2*pow(
                            sinfo->J, (Real)(-4.0/3.0))
