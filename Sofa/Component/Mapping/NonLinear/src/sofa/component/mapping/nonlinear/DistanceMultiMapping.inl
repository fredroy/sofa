    ...
    sofa::core::GeometricStiffnessMatrix* matrices)
{
    ...
    for(size_t i=0; i<links.size(); i++)
    {
        ...
        if( childForce[i][0] < 0 || geometricStiffness==1 )
        {
            ...
            auto dJ0_dX0 = matrices->getMappingDerivativeIn(this->getFromModels()[pair0[0]]).withRespectToPositionsIn(this->getFromModels()[pair0[0]]);
            auto dJ0_dX1 = matrices->getMappingDerivativeIn(this->getFromModels()[pair0[0]]).withRespectToPositionsIn(this->getFromModels()[pair1[0]]);
            auto dJ1_dX0 = matrices->getMappingDerivativeIn(this->getFromModels()[pair1[0]]).withRespectToPositionsIn(this->getFromModels()[pair0[0]]);
            auto dJ1_dX1 = matrices->getMappingDerivativeIn(this->getFromModels()[pair1[0]]).withRespectToPositionsIn(this->getFromModels()[pair1[0]]);

            dJ0_dX0.checkValidity(this);
            dJ0_dX1.checkValidity(this);
            dJ1_dX0.checkValidity(this);
            dJ1_dX1.checkValidity(this);

            dJ0_dX0(pair0[1], pair0[1]) += b;
            dJ0_dX1(pair0[1], pair1[1]) += -b;
            dJ1_dX0(pair1[1], pair0[1]) += -b;
            dJ1_dX1(pair1[1], pair1[1]) += b;
        }
    }
}
