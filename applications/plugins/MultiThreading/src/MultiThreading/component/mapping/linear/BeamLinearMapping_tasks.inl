...
BeamLinearMapping_mt< TIn, TOut>::applyTask::applyTask( sofa::simulation::CpuTask::Status* status )
    : CpuTask( status )
    , _mapping(0)
    , _in(0)
    , _out(0)
...
template <class TIn, class TOut>
sofa::simulation::Task::MemoryAlloc BeamLinearMapping_mt< TIn, TOut>::applyTask::run()
{
    for (size_t i = _firstPoint; i < _lastPoint; ++i )
    {
        Coord inpos = _mapping->points[i];
        int in0 = sofa::helper::rfloor(inpos[0]);
        if (in0<0)
        in0 = 0;
...
template <class TIn, class TOut>
BeamLinearMapping_mt< TIn, TOut>::applyJTask::applyJTask( sofa::simulation::CpuTask::Status* status )
    : CpuTask( status )
    , _mapping(0)
    , _in(0)
    , _out(0)
...
template <class TIn, class TOut>
sofa::simulation::Task::MemoryAlloc BeamLinearMapping_mt< TIn, TOut>::applyJTask::run()
{
    for (size_t i = _firstPoint; i < _lastPoint; ++i )
    {
        ...
        sofa::type::Vec<N, typename In::Real> inpos = _mapping->points[i];
        int in0 = sofa::helper::rfloor(inpos[0]);
        if (in0<0)
        in0 = 0;
        else if (in0 > (int)_in->size()-2)
...
template <class TIn, class TOut>
BeamLinearMapping_mt< TIn, TOut>::applyJTmechTask::applyJTmechTask( sofa::simulation::CpuTask::Status* status )
    : CpuTask( status )
    , _mapping(0)
    , _in(0)
    , _out(0)
...
template <class TIn, class TOut>
sofa::simulation::Task::MemoryAlloc BeamLinearMapping_mt< TIn, TOut>::applyJTmechTask::run()
{
    for (size_t i = _firstPoint; i < _lastPoint; ++i )
    {
